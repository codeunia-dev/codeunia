...
import { Building2, FileText, Search, MoreHorizontal, Eye, Download } from "lucide-react";
...
export default function AdminCollaborationPage() {
  ...
  const [selectedApp, setSelectedApp] = useState<CollaborationApplication | null>(null);
  ...
  return (
    <div className="bg-black space-y-8 md:space-y-14 min-h-screen px-4 py-8 md:px-8 lg:px-16 relative overflow-x-hidden">
      ...
      <div className="flex items-center gap-3 pb-6 border-b border-zinc-800/60 relative z-10 mt-2 mb-4">
        <span className="inline-block w-2 h-6 sm:h-8 bg-gradient-to-b from-blue-400 to-indigo-400 rounded-full mr-2" />
        <div>
          ...
          <p className="text-zinc-400 mt-1 font-medium text-sm sm:text-base">View and manage all collaboration applications</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-8">
        ...
        <div className="flex flex-1 gap-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by organization, contact, email, or website..."
              value={searchTerm}
      ...
      {/* Table */}
      <Card className="border-0 shadow-2xl rounded-2xl bg-gradient-to-br from-blue-100/80 to-indigo-200/60 dark:from-blue-900/60 dark:to-indigo-800/40 relative overflow-hidden group mt-8">
        <CardHeader>
          ...
          <CardDescription className="text-zinc-500 dark:text-zinc-300 font-medium text-sm">
            Search and filter through all collaboration applications
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="flex justify-center py-12">
              <span className="animate-spin rounded-full h-8 w-8 border-2 border-blue-400 border-t-transparent"></span>
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-xs sm:text-sm hidden md:table-cell">Website</TableHead>
                ...
                <TableBody>
                  {filteredApplications.map((app) => (
                    <TableRow key={app.id} className="hover:bg-blue-700/10 transition-colors">
                      <TableCell>
                        <div className="font-medium text-sm text-zinc-900 dark:text-zinc-100">
                          {app.organization_name}
                        </div>
                      </TableCell>
                      <TableCell className="text-xs">{app.contact_person}</TableCell>
                      <TableCell className="text-xs">{app.email}</TableCell>
                      <TableCell>{getStatusBadge(app.status)}</TableCell>
                      <TableCell className="hidden md:table-cell text-xs">{app.website}</TableCell>
                      <TableCell className="hidden md:table-cell text-xs truncate max-w-xs">{app.collaboration_reason}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="h-8 w-8 p-0 hover:bg-blue-700/20 text-blue-400 font-semibold text-xs sm:text-sm">
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-md w-full max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Collaboration Application Details</DialogTitle>
            <DialogDescription>
              Full details for {selectedApp?.organization_name}
            </DialogDescription>
          </DialogHeader>
          {selectedApp && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-sm font-medium">{selectedApp.organization_name}</div>
                </div>
                <div>
                  <Label>Contact Person</Label>
                  <div className="text-sm font-medium">{selectedApp.contact_person}</div>
                </div>
                <div>
                  <Label>Email</Label>
              ...
              <div>
                <Label>Collaboration Reason</Label>
                <div className="text-sm font-medium whitespace-pre-line">{selectedApp.collaboration_reason}</div>
              </div>
              <div>
                <Label>Status</Label>
