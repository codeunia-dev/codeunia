...
export default function EventDetailPage() {
  ...
  const renderAbout = () => (
    <div className="space-y-6">
      ...
      {/* Sponsors Grid */}
      <RotatingSponsorsGrid sponsors={event?.sponsors} />
    </div>
  )
      ...
      {event?.rules && (
        <div className="bg-background/50 backdrop-blur-sm p-6 rounded-2xl border border-primary/10 shadow mb-16">
          ...
          <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
            {event.rules.map((rule: string) => <li key={rule}>{rule}</li>)}
          </ul>
        </div>
      )}
    </div>
  ...
  const renderPrizes = () => (
    <div className="space-y-6">
      <div className="bg-background/50 backdrop-blur-sm p-8 rounded-2xl border border-primary/10 shadow-xl">
        ...
        <div className="text-3xl font-bold text-primary mb-2">{event?.prize}</div>
        <p className="text-muted-foreground">{event?.prize_details || 'Exciting rewards, sponsor goodies, and recognition.'}</p>
      </div>
    </div>
  )
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  {event?.organizer && (
                    <div className="text-base md:text-lg text-muted-foreground font-bold mt-1">by {event.organizer}</div>
                  )}
                  <p className="text-lg md:text-xl text-muted-foreground leading-relaxed">{event?.excerpt}</p>
                  <div className="flex items-center gap-3 flex-wrap mt-2">
                    <Badge className={`${getCategoryColor(event?.category || '')} shadow-lg`} variant="secondary">{event?.category}</Badge>
                    {event?.featured && (<Badge className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white border-0 shadow-lg"><Star className="h-3 w-3 mr-1" />Featured</Badge>)}
                    <Badge className={`${getStatusColor(event?.status || '')} shadow-lg`} variant="secondary">{event?.status}</Badge>
                  </div>
                </div>
                {/* Registration Card (mobile only) */}
                {event?.registration_required && event?.status === 'live' && (
                  <div className="block lg:hidden mb-4">
                    <div className="bg-white dark:bg-background border border-primary/10 rounded-xl shadow-md p-6">
                      ...
                      <div className="flex items-center gap-2 text-muted-foreground mb-1">
                        ...
                        <span>{event?.registered ?? 0} participants registered</span>
                      </div>
                      ...
                      {isAuthenticated ? (
                        ...
                        <Button className="w-full bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg mt-2" asChild>
                          <Link href={`/auth/signin?returnUrl=${encodeURIComponent(`/events/${slug}`)}`}>
                            Sign in to register
                          </Link>
                        </Button>
                      )}
              ...
              <div className="space-y-6">
                ...
                {/* Event Details Card */}
                <div className="bg-white dark:bg-background border border-primary/10 rounded-xl shadow-md p-6">
                  <div className="text-lg font-semibold mb-4">Event Details</div>
                  <div className="mb-2">
                    <span className="block text-xs text-muted-foreground">Event Type</span>
                    <span className="font-medium">{event?.eventType?.join(', ') || '-'}</span>
                  </div>
                  <div className="mb-2">
                    <span className="block text-xs text-muted-foreground">Team Size</span>
                    <span className="font-medium">{event?.teamSize ? (Array.isArray(event.teamSize) ? `${event.teamSize[0]}-${event.teamSize[1]}` : event.teamSize) : '-'}</span>
                  </div>
                  <div className="mb-2">
                    <span className="block text-xs text-muted-foreground">Date</span>
                    <span className="font-medium">{event?.date ? new Date(event.date).toLocaleDateString() : '-'}</span>
                  </div>
                  <div className="mb-2">
                  ...
                  <div className="mb-2">
                    ...
                    <span className="font-medium">{event?.prize || event?.price || '-'}</span>
                  </div>
                  <div>
